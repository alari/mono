...
object ArticlesDoobieInterpreter {
  ...
  def updateIsDraft(id: Int, value: Boolean): Update0 =
    sql"UPDATE articles SET is_draft=$value, version=version+1 WHERE id=$id".update
  ...
  override def apply[A](fa: ArticleOp[A]): Task[A] = (fa match {
    case CreateArticle(user, title, createdAt) ⇒
      insertArticle(NonEmptyList.of(user), title, createdAt).transact(xa).map { id ⇒
        Article(id, NonEmptyList.of(user), title, None, None, Nil, createdAt, createdAt, None, 0, draft = true)
      }
    ...
    case PublishDraft(i) ⇒
      // TODO: update published_year, if it's not present?
      (for {
        _ ← updateIsDraft(i, value = false).run
        a ← getArticle(i).unique
      } yield a).transact(xa)

    case DraftArticle(i) ⇒
      (for {
        _ ← updateIsDraft(i, value = true).run
        a ← getArticle(i).unique
      } yield a).transact(xa)
